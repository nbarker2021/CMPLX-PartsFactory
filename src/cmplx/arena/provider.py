...
#: The 17 canonical training modules.
TRAINING_SYSTEMS: Dict[str, Dict[str, Any]] = {
    "assembly_dtt": {
        "name": "AssemblyLine / DTT",
        "spec": "AssemblyLine_DTT_spec.html",
        "min_party": 2,
        "max_party": 6,
        "base_cost": 5.0,
    ...
    "workshop": {
        "name": "Workshop / ShopClass",
        "spec": "Workshop_ShopClass_spec.html",
        "min_party": 2,
        "max_party": 8,
        "base_cost": 6.0,
...
class ArenaProvider:
    """Competitive evaluation provider: parties, modules, sessions, leaderboard.

    Persists to SQLite (standard library only).  No external dependencies.
    """
    ...
    def _maybe_close(self, c: sqlite3.Connection) -> None:
    ...
    def get_leaderboard(self, module_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        c = self._conn()
        rows = c.execute(
    ...
    def get_signup_queue(self, module_id: str) -> List[Dict[str, Any]]:
        c = self._conn()
        rows = c.execute(
    ...
    def economy_stats(self) -> Dict[str, Any]:
        c = self._conn()
        total_fees = c.execute(
            "SELECT COALESCE(SUM(ABS(amount)), 0) FROM economy WHERE txn_type = 'entry_fee'"
        ).fetchone()[0]
        total_rewards = c.execute(
            "SELECT COALESCE(SUM(amount), 0) FROM economy WHERE txn_type = 'reward'"
        ).fetchone()[0]
        self._maybe_close(c)
        return {
    ...
    def signup(self, agent_id: str, module_id: str) -> Dict[str, Any]:
        c = self._conn()
        c.execute(
            """INSERT OR REPLACE INTO signup_queue
            (agent_id, module_id, signed_up_at, status) VALUES (?,?,?,?)""",
    ...
    def form_party(self, module_id: str) -> Optional[Dict[str, Any]]:
        ...
        session_id = hashlib.sha256(
            f"session:{module_id}:{time.time()}".encode()
        ).hexdigest()[:16]
        ...
        for aid in party:
            c.execute(
                "UPDATE signup_queue SET status = 'in_session' WHERE agent_id = ? AND module_id = ?",
                (aid, module_id),
            )

        c.execute(
            "UPDATE modules SET population = population + ?, total_sessions = total_sessions + 1 WHERE module_id = ?",
            (len(party), module_id),
        )
        ...
        for aid in party:
            txn_id = hashlib.sha256(
                f"fee:{aid}:{session_id}".encode()
            ).hexdigest()[:16]
            c.execute(
                "INSERT INTO economy (txn_id, module_id, agent_id, amount, txn_type, created_at) VALUES (?,?,?,?,?,?)",
                (txn_id, module_id, aid, -cost, "entry_fee", time.time()),
            )
    ...
    def complete_session(
        ...
        c.execute(
            "UPDATE sessions SET status = 'completed', results = ?, completed_at = ? WHERE session_id = ?",
            (json.dumps(results), time.time(), session_id),
        )

        c.execute(
            "UPDATE modules SET population = MAX(0, population - ?), production = production + ? WHERE module_id = ?",
            (len(party), results.get("production_delta", 1.0), module_id),
        )

        for aid in party:
            agent_score = results.get("agent_scores", {}).get(aid, 0)
            c.execute(
                """INSERT OR REPLACE INTO leaderboard
                (agent_id, module_id, score, sessions_completed, updated_at)
            ...
            c.execute(
                "UPDATE signup_queue SET status = 'completed' WHERE agent_id = ? AND module_id = ?",
                (aid, module_id),
            )
